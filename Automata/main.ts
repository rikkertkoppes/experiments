...
function resize(canvas: HTMLElement, renderer: THREE.Renderer, camera: THREE.OrthographicCamera) {
...
function initRenderer(canvas: HTMLCanvasElement): THREE.Renderer {
...
function initCamera(w: number,h: number): THREE.OrthographicCamera {
    var camera = new THREE.OrthographicCamera(-w/2, w/2, -h/2, h/2, 1, 1000 );
    camera.position.z = 1;
    return camera;
}

function initGeometry(): THREE.Geometry {
...
function initMaterial(attributes: Object): THREE.ShaderMaterial {
...
function init(canvasId: string) {
    ...
    var renderer = initRenderer(<HTMLCanvasElement> canvas);
    var scene = initScene();
